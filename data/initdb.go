...
import (
	...
	"github.com/Mathis-Pain/Forum/internal/utils/builddb"
	"github.com/Mathis-Pain/Forum/internal/utils/dbutils"
)
...
func InitDB(dbPath string, schemaPath string) (*sql.DB, error) {
	...
	if !dbExists {
		...
		if err := builddb.CompareDB(dbPath, expectedSchema); err != nil {
			fmt.Println("Schéma différent :", err)

			// Faire le backup uniquement si le schéma est différent
			if err := builddb.BackupDB(dbPath); err != nil {
	...
	if recreateDB {
		...
		builddb.DefaultDatabase(db, dbPath)
		return db, nil
	}
